 ...
 *             schema:
 *               type: string
 *               example: FCM token must not be empty
 *       404:
 *         description: User not found
 *         content:
 *           text/plain:
 ...
 *         description: Server error
 */
router.post("/users/update-fcm", verifyToken, async (req, res) => {
  try {
    const { fcmToken } = req.body;
    await userService.updateFcmToken(fcmToken, req.userId);
    res.status(200).json({ message: "FCM token updated successfully" });
  } catch (error) {
    console.error("Lỗi trong tuyến đường /update-fcm:", error, error.stack);
    if (typeof error === "string") {
