...
const redisClient = redis.createClient({
  ...
  retry_strategy: (options) => {
    if (options.error && options.error.code === "ECONNREFUSED") {
      ...
      return Math.min(options.attempt * 100, 3000); // Retry every 100ms, max 3s
    }
...
async function ensureRedisConnection() {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      attempts++;
      console.error(`Lỗi kết nối Redis (thử lại ${attempts}/${maxAttempts}):`, error.message);
      if (attempts === maxAttempts) {
        console.error("Không thể kết nối Redis sau nhiều lần thử, thoát...");
