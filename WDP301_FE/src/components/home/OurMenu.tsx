...
const OurMenu: React.FC = () => {
  ...
  const [selectedMainProductForModal, setSelectedMainProductForModal] = useState<Product | null>(null);
  const initialModalAddonCategories: AddOnCategory[] = [
    { id: 'modal_drinks', title: 'Nước uống', selectionText: 'Tùy chọn', apiType: 2, items: [], loading: true },
    { id: 'modal_sideDishes', title: 'Món ăn kèm', selectionText: 'Tùy chọn', apiType: 3, items: [], loading: true },
    { id: 'modal_soups', title: 'Canh ăn kèm', selectionText: 'Tùy chọn', apiType: 4, items: [], loading: true },
  ];
  const [modalAddonCategories, setModalAddonCategories] = useState<AddOnCategory[]>(JSON.parse(JSON.stringify(initialModalAddonCategories)));
  ...
  const fetchModalCategoryItems = useCallback(async (categoryIndex: number) => {
    ...
    if (categoryToFetch.apiType === 2 && drinks.length > 0) {
        ...
        try {
            const response = await axios.get(`https://wdp-301-0fd32c261026.herokuapp.com/api/products/type/${categoryToFetch.apiType}`);
  ...
  useEffect(() => {
    if (isModalVisible && selectedMainProductForModal) {
      const freshCategories = JSON.parse(JSON.stringify(initialModalAddonCategories));
  ...
  }, [isModalVisible, selectedMainProductForModal, modalAddonCategories, fetchModalCategoryItems]);
  ...
  const handleModalQuantityChange = (categoryIndex: number, itemIndex: number, change: number) => {
  ...
  const handleAddToCartFromModal = () => {
    ...
    message.success(`${selectedMainProductForModal.name} và các món kèm đã được thêm vào giỏ!`);
    handleCloseModal();
  };

  const itemsToDisplay = activeTab === 'mainDishes' ? mainDishes : activeTab === 'sideDishes' ? sideDishes : drinks;
  ...
  const tabButtonStyle = (tabName: string) => ({
    ...
    boxShadow: activeTab === tabName ? '0 4px 14px rgba(249, 115, 22, 0.3)' : 'none',
    outline: 'none',
  });
  ...
  return (
    <div style={{ padding: '20px 0 50px 0', background: '#fff7e6' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 40 }}>
        ...
        <Button style={tabButtonStyle('drinks')} onClick={() => setActiveTab('drinks')}>ĐỒ UỐNG</Button>
      </div>

      <Row gutter={[24, 32]} justify="center" style={{ maxWidth: 1200, margin: '0 auto' }}>
        {(activeTab === 'mainDishes' && loadingMain) || (activeTab === 'drinks' && loadingDrinks) || (activeTab === 'sideDishes' && loadingSide) ? (
          <Col span={24} style={{ textAlign: 'center', padding: '50px 0' }}><Spin size="large" tip="Đang tải thực đơn..." /></Col>
        ) : itemsToDisplay.length === 0 ? (
          <Col span={24}><p style={{ textAlign: 'center', fontSize: 16, color: '#666' }}>Không có món nào trong danh mục này.</p></Col>
        ) : (
          itemsToDisplay.slice(0, 4).map((item) => {
            const displayDescription = (item.description && item.description.trim() !== '' && item.description.trim().toLowerCase() !== item.name.trim().toLowerCase())
              ? item.description
                ...
                >
                  <div style={{ position: 'relative', width: '100%', paddingTop: '75%', overflow: 'hidden' }}>
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = 'https://via.placeholder.com/400x300/FDFCFB/CCC?text=Ảnh+món';
                      }}
                    />
                  </div>
                  ...
                  <div style={{ padding: '16px', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Text style={{ fontSize: '1.25rem', fontWeight: 'bold', color: '#e67e22' }}>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (activeTab === 'mainDishes') {
                            ...
                            console.log(`Trực tiếp thêm ${item.name} vào giỏ hàng.`);
                            message.success(`${item.name} đã được thêm vào giỏ (ví dụ).`);
                          }
                        }}>Thêm vào giỏ</Button>
      ...
      {selectedMainProductForModal && (
        <Modal centered open={isModalVisible} onCancel={handleCloseModal} footer={null} width={600} bodyStyle={{ padding: 0, backgroundColor: '#F8F0E5' }} closable={false} destroyOnClose >
          <div style={{ padding: '20px' }}>
            <Button icon={<CloseOutlined />} onClick={handleCloseModal} style={{ position: 'absolute', top: 16, right: 16, zIndex: 10, border: 'none', background: 'transparent', fontSize: '18px', color: '#777' }} />
            <Row gutter={16} align="middle" style={{ marginBottom: '20px' }}>
              <Col><img src={selectedMainProductForModal.image} alt={selectedMainProductForModal.name} style={{ width: '100px', height: '100px', objectFit: 'cover', borderRadius: '8px' }} /></Col>
              <Col flex="auto">
                <Title level={4} style={{ margin: 0, color: '#333' }}>{selectedMainProductForModal.name}</Title>
                {selectedMainProductForModal.description && <Text type="secondary" style={{ fontSize: '13px' }}>{selectedMainProductForModal.description.length > 60 ? selectedMainProductForModal.description.slice(0, 60) + '...' : selectedMainProductForModal.description}</Text>}
              </Col>
              <Col><Tag color="#f97316" style={{ fontSize: '16px', padding: '6px 12px', fontWeight: 'bold' }}>{parseFloat(selectedMainProductForModal.price).toLocaleString()}đ</Tag></Col>
            </Row>
            <Divider style={{ margin: '0 0 20px 0' }} />
            <div style={{ maxHeight: 'calc(100vh - 420px)', overflowY: 'auto', paddingRight: '10px' }}>
              {modalAddonCategories.map((category, catIndex) => (
                <div key={category.id} style={{ marginBottom: '24px' }}>
                  <Title level={5} style={{ marginBottom: '4px', color: '#4A4A4A', fontWeight: 'bold', marginTop: '6px' }}> {category.title} <Text type="secondary" style={{ marginLeft: '8px', fontSize: '13px', fontWeight: 'normal' }}>({category.selectionText})</Text> </Title>
                  <Divider style={{ margin: '8px 0 12px 0' }} />
                  {category.loading ? (<div style={{ textAlign: 'center', padding: '20px' }}><Spin tip={`Đang tải ${category.title.toLowerCase()}...`} /></div>) : category.items.length === 0 ? (<Text>Không có lựa chọn nào cho mục này.</Text>) : (
                    category.items.map((item, itemIndex) => (
                      <Row key={item.productId} justify="space-between" align="middle" style={{ marginBottom: '12px', padding: '8px', background: '#fff', borderRadius: '6px'}}>
                        <Col> <Text style={{ fontSize: '15px', color: '#333' }}>{item.name}</Text> <br /> <Text style={{ fontSize: '14px', color: '#f97316', fontWeight: 500 }}> + {parseFloat(item.price).toLocaleString()}đ </Text> </Col>
                        <Col>
                          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #d9d9d9', borderRadius: '4px' }}>
                            <Button icon={<MinusOutlined />} onClick={() => handleModalQuantityChange(catIndex, itemIndex, -1)} type="text" style={{ borderRight: '1px solid #d9d9d9', borderRadius: '4px 0 0 4px', padding: '0 10px', height: '30px', outline: 'none'}} disabled={item.selectedQuantity === 0} />
                            <Text style={{ padding: '0 12px', fontSize: '14px', height: '30px', lineHeight: '30px', minWidth: '20px', textAlign: 'center' }}>{item.selectedQuantity}</Text>
                            <Button icon={<PlusOutlined />} onClick={() => handleModalQuantityChange(catIndex, itemIndex, 1)} type="text" style={{ borderLeft: '1px solid #d9d9d9', borderRadius: '0 4px 4px 0', padding: '0 10px', height: '30px' , outline: 'none' }} disabled={item.selectedQuantity >= 10} />
                          </div>
                        </Col>
                      </Row>
                    ))
          ...
          <div style={{ padding: '16px 20px', borderTop: '1px solid #e8e8e8', background: '#fff', position: 'sticky', bottom: 0, zIndex: 1 }}>
            <Button type="primary" block style={{ backgroundColor: '#6A994E', borderColor: '#6A994E', height: '48px', fontSize: '16px', fontWeight: 'bold' }} onClick={handleAddToCartFromModal} > {modalTotalPrice.toLocaleString()}đ - Thêm vào giỏ hàng </Button>
          </div>
        </Modal>
      )}
    </div>
