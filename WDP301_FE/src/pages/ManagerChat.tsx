...
const ManagerChat = () => {
  ...
  const { mutate: createChat, isLoading: isSending } = useCreateChat();
  ...
  const connectSocket = () => {
    if (!token || !authUser?.id) {
      console.error("❌ No token or user ID available", { token: !!token, userId: authUser?.id });
    ...
    newSocket.on("connect", () => {
      console.log("✅ Connected to WebSocket server", { userId: authUser.id, socketId: newSocket.id });
      setIsConnected(true);
      ...
      setChats((prevChats) => {
        ...
        const newMessage = {
          ...
          Sender: receivedMessage.Sender || {
            ...
            fullName: accounts?.find((acc) => acc.id === receivedMessage.senderId)?.fullName || "Unknown",
          },
          Receiver: receivedMessage.Receiver || {
            id: receivedMessage.receiverId,
            fullName: accounts?.find((acc) => acc.id === receivedMessage.receiverId)?.fullName || "Unknown",
          },
        };
        ...
        setChats(
          filteredChats
            .map((chat) => ({
              ...
              Sender: chat.Sender || {
                id: chat.senderId,
                fullName: accounts?.find((acc) => acc.id === chat.senderId)?.fullName || "Unknown",
              },
              Receiver: chat.Receiver || {
                id: chat.receiverId,
                fullName: accounts?.find((acc) => acc.id === chat.receiverId)?.fullName || "Unknown",
              },
            }))
