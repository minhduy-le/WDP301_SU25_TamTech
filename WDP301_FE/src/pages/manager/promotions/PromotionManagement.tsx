...
import {
  ...
  SearchOutlined,
  EyeOutlined,
  CalendarOutlined, // Icon cho ngày tháng
  PercentageOutlined, // Icon cho phần trăm
  DollarCircleOutlined, // Icon cho tiền tệ
...
dayjs.tz.setDefault(dayjs.tz.guess()); // Hoặc đặt múi giờ cụ thể: "Asia/Ho_Chi_Minh"

const { Option } = Select;
const { RangePicker } = DatePicker;

// Định nghĩa interface Promotion (đã định nghĩa ở trên, copy lại cho context)
interface Promotion {
  promotionId: string;
  name: string;
  description: string;
...
const PromotionManagement: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<string>("Tất cả"); // 'Tất cả', 'Đang diễn ra', 'Sắp diễn ra', 'Đã kết thúc', 'Không hoạt động'
  const [modalVisible, setModalVisible] = useState(false);
  const [editingPromotion, setEditingPromotion] = useState<Promotion | null>(null);
  const [selectedPromotion, setSelectedPromotion] = useState<Promotion | null>(null);
  const [modalMode, setModalMode] = useState<"view" | "add" | "edit">("view");
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false); // Giả lập loading, không có API call thực tế
  ...
  const getPromotionStatus = (promotion: Promotion): { text: string; color: string } => {
    ...
    if (!promotion.isActive) {
      return { text: "Không hoạt động", color: "grey" };
    }
    if (now.isAfter(endDate)) {
      return { text: "Đã kết thúc", color: "red" };
    }
    if (now.isBefore(startDate)) {
      return { text: "Sắp diễn ra", color: "geekblue" };
    }
  ...
  const handleAdd = () => {
    ...
    setModalMode("add");
    form.resetFields();
    form.setFieldsValue({ isActive: true, discountType: 'percentage' }); // Giá trị mặc định
    ...
    try {
      ...
      if (modalMode === "add") {
        const newPromotion: Promotion = {
          ...promotionPayload,
          promotionId: `PROMO${Date.now()}`, // Tạo ID tạm
        };
        ...
        setPromotions((prev) =>
          prev.map((p) => (p.promotionId === editingPromotion.promotionId ? updatedPromotion : p))
        );
  ...
  const columns = [
    {
      title: <span style={{ color: "#A05A2C", fontWeight: "bold" }}>Tên khuyến mãi</span>,
      dataIndex: "name",
      key: "name",
      width: 200,
      ellipsis: true,
    ...
    {
      title: <span style={{ color: "#A05A2C", fontWeight: "bold" }}>Mã</span>,
      dataIndex: "code",
      key: "code",
      width: 120,
      ellipsis: true,
      render: (code?: string) => code ? <Tag color="blue">{code}</Tag> : <span style={{color: "#aaa"}}>Không có</span>,
    },
    {
      title: <span style={{ color: "#A05A2C", fontWeight: "bold" }}>Giảm giá</span>,
    ...
    {
      title: <span style={{ color: "#A05A2C", fontWeight: "bold" }}>Thời gian</span>,
      key: "duration",
      width: 220,
    ...
    },
    {
      ...
      render: (_:any, record: Promotion) => {
        ...
        return <Tag color={status.color} style={{ fontWeight: 500, padding: "3px 8px", borderRadius: "6px" }}>{status.text}</Tag>;
      }
    },
    {
      ...
      render: (_: any, record: Promotion) => (
        <Space size="small">
          <Tooltip title="Xem chi tiết">
            <Button type="text" icon={<EyeOutlined style={{ color: "#D97B41", fontSize: 17 }} />} onClick={() => handleView(record)} />
          </Tooltip>
          <Tooltip title="Chỉnh sửa">
            <Button type="text" icon={<EditOutlined style={{ color: "#1890ff", fontSize: 17 }} />} onClick={() => handleEdit(record)} />
          </Tooltip>
          <Tooltip title="Xóa">
            <Button type="text" danger icon={<DeleteOutlined style={{ fontSize: 17 }} />} onClick={() => handleDelete(record.promotionId)} />
          </Tooltip>
        </Space>
      ),
    },
  ];
  const statusFilterOptions = ["Tất cả", "Đang diễn ra", "Sắp diễn ra", "Đã kết thúc", "Không hoạt động"];
        ...
        >
          <div style={{ marginBottom: 20, display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <Button
              type="primary"
              icon={<PlusOutlined />}
              style={{ background: "#D97B41", borderColor: "#D97B41", fontWeight: 600, borderRadius: 6, boxShadow: "0 2px 0 rgba(0,0,0,0.043)" }}
              onClick={handleAdd}
            >
              Thêm khuyến mãi
            </Button>
        ...
        <Modal
          ...
          footer={
            modalMode === "view"
              ...
              : [
                  <Button key="cancel" onClick={() => { setModalVisible(false); form.resetFields(); }} style={{ borderRadius: 6 }}>Hủy</Button>,
                  <Button key="submit" type="primary" onClick={handleModalOk} style={{ background: "#D97B41", borderColor: "#D97B41", borderRadius: 6 }}>
                    {modalMode === "add" ? "Thêm mới" : "Cập nhật"}
                  </Button>,
                ]
          }
          width={modalMode === 'view' ? 700 : 650}
          destroyOnHidden // Sử dụng destroyOnHidden thay cho destroyOnClose
          styles={{ // Sử dụng styles.body thay cho bodyStyle
        ...
        >
          {modalMode === 'view' && selectedPromotion ? (
             <Card bordered={false} style={{ background: "#fff", borderRadius: 8, padding: 0 }}>
                <Descriptions bordered column={{ xxl: 1, xl: 1, lg: 1, md: 1, sm: 1, xs: 1 }} size="default"
                    labelStyle={{ color: "#A05A2C", fontWeight: 500, width: '180px' }} contentStyle={{ color: "#555" }}>
                    ...
                    <Descriptions.Item label="Trạng thái (Admin)">
                     ...
                     <Descriptions.Item label="Trạng thái thực tế">
                        <Tag color={getPromotionStatus(selectedPromotion).color}>{getPromotionStatus(selectedPromotion).text}</Tag>
                    </Descriptions.Item>
                </Descriptions>
            </Card>
          ) : (
