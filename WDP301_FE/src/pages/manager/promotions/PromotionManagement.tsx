...
const PromotionManagement: React.FC = () => {
  ...
  const fetchPromotions = useCallback(async () => {
    ...
    try {
      ...
      const mappedPromotions: Promotion[] = response.data.map(p => {
        const type = promotionTypes.find(t => t.promotionTypeId === p.promotionTypeId);
        return {
  ...
  const getPromotionStatus = (promotion: Promotion): { text: string; color: string } => {
    const now = dayjs();
    const startDate = dayjs(promotion.startDate);
    const endDate = dayjs(promotion.endDate);
    if (!promotion.isActive) return { text: "Không hoạt động", color: "#B0A8A8" };
    if (now.isAfter(endDate)) return { text: "Đã kết thúc", color: "#E57373" };
    if (now.isBefore(startDate)) return { text: "Sắp diễn ra", color: "#64B5F6" };
  ...
  const handleFormSubmit = async (values: PromotionFormValues) => {
    ...
    try {
        ...
        message.error(modalMode === 'edit' ? 'Cập nhật khuyến mãi thất bại!' : 'Tạo khuyến mãi thất bại!');
    } finally {
        setIsLoading(false);
    }
  };
  ...
  const filteredPromotions = useMemo(() => {
    return promotions.filter(promo =>
        ...
        (promo.code && promo.code.toLowerCase().includes(searchText.toLowerCase()))) &&
        (statusFilter === "Tất cả" || getPromotionStatus(promo).text === statusFilter)
    );
  ...
  const columns = [
    {
      ...
      render: (name: string) => <Tooltip title={name}><span style={{ fontWeight: 600, color: "#D97B41" }}>{name}</span></Tooltip>,
    },
    {
      title: "Mã",
      dataIndex: "code",
      ...
      width: 130,
      ellipsis: true,
      render: (code?: string) => code ? <Tag color="#A05A2C" style={{color: '#fff', fontWeight: 500}}>{code}</Tag> : <span style={{ color: "#aaa" }}>Không có</span>,
    },
    {
      title: "Giảm giá",
    ...
    },
    {
      ...
      key: "duration",
      width: 200,
      sorter: (a: Promotion, b: Promotion) => dayjs(a.startDate).diff(dayjs(b.startDate)),
      render: (_: any, record: Promotion) => (
        <div>
          <span style={{ display: "block", fontSize: "13px" }}>BĐ: {dayjs(record.startDate).format("DD/MM/YYYY HH:mm")}</span>
          <span style={{ display: "block", fontSize: "13px" }}>KT: {dayjs(record.endDate).format("DD/MM/YYYY HH:mm")}</span>
        </div>
      ),
    },
    {
      ...
      onFilter: (value: string | number | boolean, record: Promotion) => getPromotionStatus(record).text === value,
      render: (_: any, record: Promotion) => {
        const status = getPromotionStatus(record);
        return <Tag color={status.color} style={{ fontWeight: 500, padding: "3px 8px", borderRadius: "6px", color: '#fff' }}>{status.text}</Tag>;
      },
    },
    {
      title: "Loại KM",
        ...
        <Space size="small">
          ...
          <Tooltip title="Ngừng hoạt động">
             <Popconfirm
                ...
                okText="Xác nhận"
                cancelText="Hủy"
                okButtonProps={{ danger: true, style: { background: '#D97B41', borderColor: '#D97B41' } }}
              >
                <Button type="text" danger icon={<CloseCircleOutlined style={{ fontSize: 16 }} />} style={{ outline: "none", boxShadow: "none", border: "none" }} />
              </Popconfirm>
          </Tooltip>
        </Space>
      ),
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFF9F0", padding: "20px 30px 30px 60px" }}>
      <style>{`
        ...
        .ant-table-filter-dropdown-btns .ant-btn-primary { background-color: #D97B41 !important; border-color: #D97B41 !important; }
        .ant-table-filter-trigger, .ant-table-filter-trigger:hover, .ant-table-filter-trigger.active,
        .ant-table-column-sorter-up.active, .ant-table-column-sorter-down.active { color: #D97B41 !important; }
      `}</style>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto" }}>
        ...
        <Card style={{ background: "#fff", borderRadius: 12, boxShadow: "0 6px 16px rgba(160, 90, 44, 0.08)", padding: "12px 24px", border: `1px solid ${tableBorderColor}`, marginBottom: 24 }}>
          <div style={{ marginBottom: 20, display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
            <Space>
              <Input placeholder="Tìm theo tên, mã KM..." prefix={<SearchOutlined style={{ color: "#A05A2C" }} />} value={searchText} onChange={(e) => setSearchText(e.target.value)} style={{ width: 280, borderRadius: 6, borderColor: "#E9C97B", height: 32, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "none"}} allowClear />
            </Space>
            <Button type="primary" icon={<PlusOutlined />} style={{ background: "#D97B41", borderColor: "#D97B41", fontWeight: 600, borderRadius: 6, boxShadow: "0 2px 0 rgba(0,0,0,0.043)", outline: 'none' }} onClick={handleAdd}>
              Thêm khuyến mãi
            </Button>
          </div>
          <Table className="promo-table" columns={columns as ColumnType<Promotion>[]} dataSource={filteredPromotions} loading={isLoading} rowKey="promotionId" style={{ borderRadius: 8, border: `1px solid ${tableBorderColor}`, overflow: 'hidden' }} rowClassName={(_, index) => (index % 2 === 0 ? 'even-row-promo' : 'odd-row-promo')} sticky />
        </Card>
        <PromotionTypeManagement />
        <Modal
          open={modalVisible}
          title={<span style={{ color: "#A05A2C", fontWeight: 600, fontSize: 22 }}>
            {modalMode === "view" ? "Chi tiết khuyến mãi" : modalMode === "add" ? "Thêm khuyến mãi mới" : "Chỉnh sửa khuyến mãi"}
          </span>}
          onCancel={() => { setModalVisible(false); form.resetFields(); }}
          footer={modalMode === "view" ? [
            <Button key="close" onClick={() => setModalVisible(false)} style={{ borderRadius: 6 }}>Đóng</Button>
          ] : [
            <Button key="cancel" onClick={() => { setModalVisible(false); form.resetFields(); }} style={{ borderRadius: 6, outline: 'none' }} disabled={isLoading}>Hủy</Button>,
            <Button key="submit" type="primary" loading={isLoading}
              form="promotion-form" htmlType="submit"
        ...
        >
          {modalMode === "view" && selectedPromotion ? (
            <Card bordered={false} style={{ background: "#fff", borderRadius: 8, padding: 0 }}>
              <Descriptions bordered column={{ xxl: 1, xl: 1, lg: 1, md: 1, sm: 1, xs: 1 }} size="default" labelStyle={{ color: "#A05A2C", fontWeight: 500, width: "180px", background: '#FFF9F0' }} contentStyle={{ color: "#555", background: '#FFFFFF' }}>
            ...
            >
              <Form.Item style={{ marginBottom: 0 }} name="name" label={<span style={{ color: "#A05A2C" }}>Tên khuyến mãi</span>} rules={[{ required: true, message: "Vui lòng nhập tên khuyến mãi!" }]}>
                <Input placeholder="Ví dụ: Giảm giá khai trương" style={{ borderRadius: 6, marginBottom: 16 }} />
              </Form.Item>
              <Form.Item style={{ marginBottom: 0 }} name="description" label={<span style={{ color: "#A05A2C" }}>Mô tả</span>} rules={[{ required: true, message: "Vui lòng nhập mô tả!" }]}>
                <Input.TextArea rows={3} placeholder="Mô tả chi tiết về chương trình khuyến mãi" style={{ borderRadius: 6, marginBottom: 16 }} />
              </Form.Item>
              <Form.Item style={{ marginBottom: 0 }} name="code" label={<span style={{ color: "#A05A2C" }}>Mã khuyến mãi</span>} rules={[{required: true, message: "Vui lòng nhập mã khuyến mãi!" }, {max: 10, message: "Mã khuyến mãi không được vượt quá 10 ký tự!"}]}>
                <Input placeholder="Ví dụ: SALE2025" style={{ borderRadius: 6, marginBottom: 16 }} />
              </Form.Item>
              <Form.Item name="promotionTypeId" label={<span style={{ color: "#A05A2C" }}>Loại khuyến mãi</span>} rules={[{ required: true, message: "Vui lòng chọn loại khuyến mãi!" }]}> 
              ...
              <Row gutter={[16, 16]}>
                ...
                <Col span={12}>
                  <Form.Item name="maxNumberOfUses" label={<span style={{ color: "#A05A2C" }}>Số lượt sử dụng tối đa</span>} rules={[{ required: true, message: "Vui lòng nhập số lượt sử dụng tối đa!" }]}>
                    <InputNumber style={{ width: "100%", borderRadius: 6 }} min={0} placeholder="Nhập số lượt tối đa" />
                  </Form.Item>
                </Col>
              </Row>
              <Form.Item
